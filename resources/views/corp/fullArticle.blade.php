@if(isset($article))
				        ...
				        <div id="content-single" class="content group">
				            <div class="hentry hentry-post blog-big group ">
				                ...
				                <div class="thumbnail">
				                    <!-- post title -->
				                    <h1 class="post-title"><a href="{{route('articles.show',['alias'=>$article->alias])}}">
				                    	{{$article->title}}
				                    </a></h1>
				                    <!-- post featured -->
				                ...
				                <div class="meta group">
				                    ...
				                    <p class="categories"><span>In: <a href="{{route('articlesCat',['cat_alias'=>$article->category->alias])}}" title="View all posts in Design" rel="category tag">{{$article->category->title}}</a></span></p>
				                    <p class="comments"><span><a href="#comments" title="Comment on This is the title of the first article. Enjoy it.">{{count($article->comments) ? count($article->comments) :'0'}} {{Lang::choice('translate.comments',count($article->comments))}}</a></span></p>
				                </div>
				                <!-- post content -->
				                <div class="the-content single group">
									{!!$article->text!!}
				                    <div class="socials">
				                        <h2 style="font-size: 20px">{{Lang::get('translate.love_it')}}</h2>
				            ...
				            <div id="comments">
				                <h3 id="comments-title">
				                    <span>{{count($article->comments) ? count($article->comments) :'0'}}</span> {{Lang::choice('translate.comments',count($article->comments))}}   
				                </h3>
				                @if(count($article->comments)>0)
				                <ol class="commentlist group">
				                @set($comm,$article->comments->groupBy('parent_id'))
				                @foreach($comm as $k => $comments)			                	
										@if($k !== 0)
											@break
										@endif
				                ...
				                <div id="respond">
				                    <h3 id="reply-title">{{Lang::get('translate.leave_and_replay')}} <small><a rel="nofollow" id="cancel-comment-reply-link" href="#respond" style="display:none;">Cancel reply</a></small></h3>
				                    @if(!Auth::check())
				                    <form action="{{route('comment.store')}}" method="post" id="commentform">
				                        ...
				                        <p class="comment-form-url"><label for="url">Website</label><input id="url" name="site" type="site" value="" size="30" /></p>
									@endif
